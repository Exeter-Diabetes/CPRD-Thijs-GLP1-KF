...
for (k in 1:n.quantiles) {
  for (i in 1:n.imp) {
    ...
    observed_ref_ckd50 <- survfit(Surv(ckd_egfr50_censtime_yrs, ckd_egfr50_censvar) ~ risk_decile, 
    ...
    EST.ref[k,i] <- observed_ref_ckd50$estimate
    SE.ref[k,i] <- observed_ref_ckd50$std.error
...
events_ref <- cohort %>%
  filter(!!sym(studydrug_var)==reference_group & ckd_egfr50_censvar==1) %>%
  group_by(risk_decile) %>%
...
## FINAL PLOT
p_uncal_bydeciles_presegfr_ref <- ggplot(data=bind_rows(empty_tick,obs_v_pred), aes(x=mean_ckd50_pred*100)) +
  ...
  scale_x_continuous(limits=c(0,100), breaks = seq(0, max_y_value, 1))+
  scale_y_continuous(limits=c(-1,100), breaks = seq(0, max_y_value, 1)) +
...
raw_mod <- coxph(Surv(ckd_egfr50_censtime_yrs, ckd_egfr50_censvar) ~ ckdpc_50egfr_survival, data=cohort[cohort[[studydrug_var]] == reference_group,], method="breslow")
...
# 
# ## FINAL PLOT
# p_uncal_bydeciles_redegfr_ref <- ggplot(data=bind_rows(empty_tick,obs_v_pred), aes(x=mean_ckd50_pred*100)) +
...
# 
# 
# ############################3 RECALIBRATION OF RISK SCORE IN REDUCED EGFR################################################################
# 
# # calculate recalibrated score
# cohort <- cohort %>% mutate(
#     ckdpc_50egfr_lin_predictor_cal=coef_recal_redegfr*ckdpc_50egfr_lin_predictor,
#     ckdpc_50egfr_survival_cal=bh_recal_redegfr^exp(ckdpc_50egfr_lin_predictor_cal-mean(ckdpc_50egfr_lin_predictor_cal)), # baseline hazard ^ e ^ centred linear predictor
#     ckdpc_50egfr_score_cal=(1-ckdpc_50egfr_survival_cal)*100
#     )
# 
# 
# 
# # make new variable for risk deciles
...
# 
# ## plot
# p_recal_bydeciles_redegfr_ref <- ggplot(data=bind_rows(empty_tick,obs_v_pred), aes(x=mean_ckd50_pred*100)) +
#   geom_errorbar(aes(ymax=upper_ci_ref*100,ymin=lower_ci_ref*100, color=!!sym(studydrug_var)),width=0.6,size=1) +
#   geom_point(aes(y = observed_ref*100, group=!!sym(studydrug_var), color=!!sym(studydrug_var)), shape=18, size=3) +
#   geom_abline(intercept = 0, slope = 1, lty = 2) +
#   theme_bw() +
...
# 
# 
# ## brier score for recalibrated risk score
# brier_raw_redegfr_recal <- rep(NA, n.imp)
# brier_raw_se_redegfr_recal <- rep(NA, n.imp)
# 
# 
# # for (i in 1:n.imp) {
# #   print(paste("Imputation ", i))
# #   
# #   temp1 <- cohort %>% filter(.imp == i) %>% select(ckd_egfr50_censtime_yrs, ckd_egfr50_censvar, ckdpc_50egfr_survival_cal)
# #   temp1 <- temp1 %>% # error if times = 3, therefore adding extra row with time beyond t=3
# #     rbind(    # adds rows below your dataset
# #       temp1 %>%
# #         slice(1) %>% # this selects the first patients in dataset
# #         mutate(ckd_egfr50_censtime_yrs = 3.5)   # changes censored time to 3.5
# #     )
# #   raw_mod <- coxph(Surv(ckd_egfr50_censtime_yrs, ckd_egfr50_censvar) ~ ckdpc_50egfr_survival_cal, 
# #                    data=temp1, x=T)
# #   
# #   
# #   score_raw <- 
...
# #           verbose = T) 
# #   
# #   brier_raw_redegfr_recal[i] <- score_raw$Brier$score$Brier[2]
# #   brier_raw_se_redegfr_recal[i] <- score_raw$Brier$score$se[2]
# #   
# #   rm(temp)
# # }
# 
# brier_raw_se_pooled_redegfr_recal <- sqrt(mean(brier_raw_se_redegfr_recal^2) + (1+1/n.imp)*var(brier_raw_redegfr_recal))
# 
# ### calibration slope
# 
# # create empty vectors to store values of each calculation in multiple imputations:
# bh_new_redegfr_recal <- rep(NA, n.imp)
# se_bh_new_redegfr_recal <- rep(NA, n.imp)
# cal_slope_redegfr_recal <- rep(NA, n.imp)
# slope_optimism_redegfr_recal <- rep(NA, n.imp)
# var_slope_redegfr_recal <- rep(NA, n.imp)
# 
# options(datadist=NULL)
# 
# for (i in 1:n.imp) {
#   print(paste0("Calculations in imputation ", i))
#   # fit model with linear predictor as only variable
#   recal_mod3 <- cph(Surv(ckd_egfr50_censtime_yrs, ckd_egfr50_censvar) ~ (ckdpc_50egfr_lin_predictor_cal), 
#                     data = cohort %>% filter(.imp == i & !!sym(studydrug_var) == reference_group), x = TRUE, y = TRUE, surv = TRUE)
#   
#   # Obtain baseline survival estimates for model
#   x <- summary(survfit(recal_mod3),time=3)
#   bh_new_redegfr_recal[i] <- x$surv
#   se_bh_new_redegfr_recal[i] <- x$std.err
#   
#   # bootstrap internal validation
#   boot <- validate(recal_mod3)
#   slope_optimism_redegfr_recal[i] <- boot[3,5]
#   
#   # store calibration slope
#   cal_slope_redegfr_recal[i] <- recal_mod3$coefficients * boot[3,5]
#   var_slope_redegfr_recal[i] <- recal_mod3$var
# }
# 
# # pool baseline hazard results
...
############################4 STORE RECALIBRATED SCORE################################################################

temp <- temp %>% filter(preegfr >= 60) %>% mutate( ckdpc_50egfr_lin_predictor_cal=ckdpc_50egfr_lin_predictor,
                                                   ckdpc_50egfr_survival_cal=ckdpc_50egfr_survival,
                                                   ckdpc_50egfr_score_cal=ckdpc_50egfr_score
                                                   )

# cohort <- rbind(temp, cohort %>% select(-risk_decile))
cohort <- temp %>% select(-risk_decile)


setwd("C:/Users/tj358/OneDrive - University of Exeter/CPRD/2024/Processed data/")
save(cohort, file=paste0(today, "_t2d_glp1_imputed_data_withweights_recalibrated.Rda"))
