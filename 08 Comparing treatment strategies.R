...
cohort <- cohort %>% mutate(ckdpc_50egfr_score.SGLT2.guideline = 
                              ifelse(ckd_cat == "≥60_<3", ckdpc_50egfr_score,
                                     100*(1-cohort$ckdpc_50egfr_survival_SGLT2))
                            )

events.SGLT2.only <- round(
  nrow(cohort) * mean(cohort$ckdpc_50egfr_score.SGLT2.guideline/100)
)

#estimated number if events if we were to treat all these people with SGLT2 + GLP1
cohort <- cohort %>% mutate(`ckdpc_50egfr_score.GLP + SGLT2.all` = 
                              ifelse(ckd_cat == "≥60_<3", ckdpc_50egfr_score,
                                     100*(1-cohort$`ckdpc_50egfr_survival_GLP + SGLT2`))
)

`events.GLP + SGLT2.all` <- round(
  nrow(cohort) * mean(cohort$`ckdpc_50egfr_score.GLP + SGLT2.all`/100)
)

#estimated number of events if we were to treat SGLT2 as per guidelines and added GLP1 only to those per FLOW criteria
cohort <- cohort %>% mutate(`ckdpc_50egfr_score.GLP + SGLT2.flow` = 
                              ifelse(flow_criteria == F, ckdpc_50egfr_score.SGLT2.guideline,
                                     100*(1-cohort$`ckdpc_50egfr_survival_GLP + SGLT2`))
)

`events.GLP + SGLT2.flow` <- round(
  nrow(cohort) * mean(cohort$`ckdpc_50egfr_score.GLP + SGLT2.flow`/100)
)

#estimated number of events if we were to treat SGLT2 as per guidelines and added GLP1 based on pARR
cohort <- cohort %>% mutate(`ckdpc_50egfr_score.GLP + SGLT2.model` = 
                              ifelse(treat_model == F, ckdpc_50egfr_score.SGLT2.guideline,
                                     100*(1-cohort$`ckdpc_50egfr_survival_GLP + SGLT2`))
)

`events.GLP + SGLT2.model` <- round(
  nrow(cohort) * mean(cohort$`ckdpc_50egfr_score.GLP + SGLT2.model`/100)
)
...
print(paste0(c("Number of people treated with SGLT2 as per guidelines only: ", 
               round(nrow(cohort[cohort$ckd_cat != "≥60_<3",])/n.imp), " (", round(nrow(cohort[cohort$ckd_cat != "≥60_<3",])/nrow(cohort)*100,1), "% of the population)"), collapse = ""))
print(paste0(c("Number of these people treated with GLP1 as per guidelines only: 0 (0% of people with CKD)"), collapse = ""))
print(paste0(c("Number of events with SGLT2 as per guidelines only: ", 
...
print(paste0(c("Number of events avoided by GLP1 with SGLT2 as per guidelines only: -")))
print(paste0(c("NNT for SGLT2 with SGLT2 treatment as per guidelines only: ", 
               round(1/(mean(cohort[cohort$ckd_cat !=  "≥60_<3",]$ckdpc_50egfr_score/100) - 
                          mean(cohort[cohort$ckd_cat !=  "≥60_<3",]$ckdpc_50egfr_score.SGLT2.only/100)))), collapse = ""))
print(paste0(c("NNT for GLP1 with SGLT2 as per guidelines only: -"), collapse = ""))
...
print(paste0(c("Number of people treated with GLP1 if treating everyone eligible for SGLT2 with GLP1 as well: ", 
               round(nrow(cohort[cohort$ckd_cat != "≥60_<3",])/n.imp), " (", round(nrow(cohort[cohort$ckd_cat != "≥60_<3",])/nrow(cohort[cohort$ckd_cat != "≥60_<3",])*100,1), "% of people with CKD)"), collapse = ""))
print(paste0(c("Number of events if treating everyone eligible for SGLT2 with GLP1 as well: ", 
               round(`events.GLP + SGLT2.all`/n.imp), " (", round(100*(`events.GLP + SGLT2.all`/nrow(cohort)),1), "% of individuals)"), collapse = ""))
print(paste0(c("Number of events avoided with SGLT2 as per guidelines only: ", 
               round(abs((events.notx - events.SGLT2.only)/n.imp)), " (", round((events.SGLT2.only-events.notx)/(events.SGLT2.only-events.notx)*100,1), "% of events preventable by SGLT2)"), collapse = ""))
print(paste0(c("Number of events avoided by GLP1 if treating everyone eligible for SGLT2 with GLP1 as well: ", 
               round(abs((events.SGLT2.only - `events.GLP + SGLT2.all`)/n.imp)), " (", round((`events.GLP + SGLT2.all`-events.SGLT2.only)/(`events.GLP + SGLT2.all`-events.SGLT2.only)*100,1), "% of additional events preventable by GLP1)"), collapse = ""))
...
print(paste0(c("NNT for GLP1 if treating everyone eligible for SGLT2 with GLP1 as well: ", 
               round(1/(mean(cohort[cohort$ckd_cat !=  "≥60_<3",]$ckdpc_50egfr_score.SGLT2.only/100) - 
                          mean(cohort[cohort$ckd_cat !=  "≥60_<3",]$`ckdpc_50egfr_score.GLP + SGLT2.all`/100)))), collapse = ""))
...
print(paste0(c("Number of people treated with GLP1 if additionally treating with GLP1 based on FLOW criteria: ", 
               round(nrow(cohort[cohort$flow_criteria == T,])/n.imp), " (", round(nrow(cohort[cohort$flow_criteria == T,])/nrow(cohort[cohort$ckd_cat != "≥60_<3",])*100,1), "% of people with CKD)"), collapse = ""))
print(paste0(c("Number of events if additionally treating with GLP1 based on FLOW criteria: ", 
               round(`events.GLP + SGLT2.flow`/n.imp), " (", round(100*(`events.GLP + SGLT2.flow`/nrow(cohort)),1), "% of individuals)"), collapse = ""))
print(paste0(c("Number of events avoided with SGLT2 as per guidelines only: ", 
               round(abs((events.notx - events.SGLT2.only)/n.imp)), " (", round((events.SGLT2.only-events.notx)/(events.SGLT2.only-events.notx)*100,1), "% of events preventable by SGLT2)"), collapse = ""))
print(paste0(c("Number of events avoided by GLP1 if additionally treating with GLP1 based on FLOW criteria: ", 
               round(abs((events.SGLT2.only - `events.GLP + SGLT2.flow`)/n.imp)), " (", round((`events.GLP + SGLT2.flow`-events.SGLT2.only)/(`events.GLP + SGLT2.all`-events.SGLT2.only)*100,1), "% of additional events preventable by GLP1)"), collapse = ""))
...
print(paste0(c("NNT for GLP1 if additionally treating with GLP1 based on FLOW criteria: ", 
               round(1/(mean(cohort[cohort$flow_criteria == T,]$ckdpc_50egfr_score.SGLT2.only/100) - 
                          mean(cohort[cohort$flow_criteria == T,]$`ckdpc_50egfr_score.GLP + SGLT2.flow`/100)))), collapse = ""))
...
print(paste0(c("Number of people treated with GLP1 if additionally treating with GLP1 based on pARR: ", 
               round(nrow(cohort[cohort$treat_model == T,])/n.imp), " (", round(nrow(cohort[cohort$treat_model == T,])/nrow(cohort[cohort$ckd_cat != "≥60_<3",])*100,1), "% of people with CKD)"), collapse = ""))
print(paste0(c("Number of events if additionally treating with GLP1 based on pARR: ", 
               round(`events.GLP + SGLT2.model`/n.imp), " (", round(100*(`events.GLP + SGLT2.model`/nrow(cohort)),1), "% of individuals)"), collapse = ""))
print(paste0(c("Number of events avoided with SGLT2 as per guidelines only: ", 
               round(abs((events.notx - events.SGLT2.only)/n.imp)), " (", round((events.SGLT2.only-events.notx)/(events.SGLT2.only-events.notx)*100,1), "% of events preventable by SGLT2)"), collapse = ""))
print(paste0(c("Number of events avoided by GLP1 if additionally treating with GLP1 based on pARR: ", 
               round(abs((events.SGLT2.only - `events.GLP + SGLT2.model`)/n.imp)), " (", round((`events.GLP + SGLT2.model`-events.SGLT2.only)/(`events.GLP + SGLT2.all`-events.SGLT2.only)*100,1), "% of additional events preventable by GLP1)"), collapse = ""))
...
print(paste0(c("NNT for GLP1 if additionally treating with GLP1 based on pARR: ", 
               round(1/(mean(cohort[cohort$treat_model == T,]$ckdpc_50egfr_score.SGLT2.only/100) - 
                          mean(cohort[cohort$treat_model == T,]$`ckdpc_50egfr_score.GLP + SGLT2.model`/100)))), collapse = ""))
