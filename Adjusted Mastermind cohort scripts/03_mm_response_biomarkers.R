...
egfr40_decline_next_egfr <- egfr40_decline %>%
  left_join(egfr_long, by="patid") %>%
  filter(datediff(date, egfr_40_decline_date)>=28) %>% 
  group_by(patid, drug_substance, dstartdate) %>%
  summarise(post_egfr_40_decline_date_next_egfr=min(date, na.rm=T)) %>% 
  ungroup() %>%
  analysis$cached("response_biomarkers_egfr40_decline_next_egfr", indexes=c("patid", "dstartdate", "drug_substance"))
...
response_biomarkers <- response_biomarkers %>%
  ...
  left_join(egfr40_decline_next_egfr, by=c("patid", "drug_substance", "dstartdate")) %>%
