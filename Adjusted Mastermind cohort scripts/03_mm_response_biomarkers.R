...
# HbA1c only: response missing where timeprevcombo_class<=61 days before drug initiation

# All biomarker tests merged with all drug start and stop dates (plus timetochange_class, timeaddrem and multi_drug_start from mm_combo_start_stop = combination start stop table) in script 2_mm_baseline_biomarkers - tables created have names of the form 'mm_full_{biomarker}_drug_merge'
...
### Latest = minimum of timetochange_class + 91 days, timetoaddrem_class and 9 months (for 6m response)/15 months (for 12m response)
...
## Remove HbA1c responses where timeprevcombo_class<=61 days i.e. where change glucose-lowering meds less than 61 days before current drug initation
...
prev_acr <- baseline_biomarkers %>%
  select(patid, drug_substance, dstartdate, preacrdate) %>%
...
new_macroalb <- baseline_biomarkers %>%
  ...
  analysis$cached("response_biomarkers_macroalb_im", indexes=c("patid", "dstartdate", "drug_substance"))
...
response_biomarkers <- response_biomarkers %>%
  ...
  # left_join(next_acr, by=c("patid", "drug_substance", "dstartdate")) %>%
