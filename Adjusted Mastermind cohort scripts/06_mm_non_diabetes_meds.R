...
#codes2 = codesets$getAllCodeSetVersion(v = "31/10/2021")
...
for (i in meds) {
  ...
  #  filter(date>=min_dob & date<=gp_ons_end_date) %>%
    ...
    inner_join((drug_start_stop %>% select(patid, dstartdate, drug_substance, drug_substance, drug_instance)), by="patid") %>%
    mutate(drugdatediff=datediff(date, dstartdate)) %>%
    
    analysis$cached(drug_merge_tablename, indexes=c("patid", "dstartdate", "drug_substance"))
...
## Remove definite_genital_infection_meds  as need to be used in combination with genital_infection_nonspec medcodes (see script 4_mm_comorbidities)

meds <- setdiff(meds, c("definite_genital_infection_meds"))

non_diabetes_meds <- drug_start_stop %>%
  select(patid, dstartdate, drug_substance, drug_instance)
